...
const logger = createLogger('LocalAIServiceBase');
...
class LocalAIServiceBase extends EventEmitter {
    ...
    async autoInstall(onProgress) {
        const platform = this.getPlatform();
        logger.info('Starting auto-installation for');
    ...
    async shutdown(force = false) {
        ...
        if (!isRunning) {
            logger.info('Service not running, nothing to shutdown');
            return true;
        }
        ...
        try {
            switch(platform) {
                ...
                default:
                    logger.warn('Unsupported platform for shutdown: ${platform}');
                    return false;
            }
        } catch (error) {
    ...
    async downloadFile(url, destination, options = {}) {
        const { 
            ...
            modelId = null // Model ID[Korean comment translated] [Korean comment translated] [Korean comment translated] [Korean comment translated]
        } = options;
        ...
        return new Promise((resolve, reject) => {
            ...
            const request = https.get(url, { headers }, (response) => {
                ...
                if ([301, 302, 307, 308].includes(response.statusCode)) {
                    ...
                    logger.info('Following redirect from  to');
                ...
                response.on('data', (chunk) => {
                    ...
                    if (totalSize > 0) {
                        ...
                        // [Korean comment translated] [Korean comment translated] [Korean comment translated] [Korean comment translated] [Korean comment translated] [Korean comment translated] [Korean comment translated] Process
                ...
                file.on('finish', () => {
                    file.close(() => {
                        // download-complete [Korean comment translated] [Korean comment translated] [Korean comment translated] [Korean comment translated] Process
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (attempt === maxRetries) {
                    // download-error [Korean comment translated] [Korean comment translated] [Korean comment translated] [Korean comment translated] Process
                    throw error;
                }
                
                logger.info(`Download attempt ${attempt} failed, retrying in ${retryDelay}ms...`);

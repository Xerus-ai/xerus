...
try {
    ...
    logger.warn('keytar is not available. Will use in-memory key for this session. Restarting the app might be required for data persistence after login.');
    keytar = null;
}

const SERVICE_NAME = 'com.xerus.app'; // A unique identifier for the app in the keychain
...
async function initializeKey(userId) {
    ...
    if (keytar) {
        try {
            ...
            if (!key) {
                logger.info('No key found for . Creating a new one.');
                key = crypto.randomBytes(32).toString('hex');
                await keytar.setPassword(SERVICE_NAME, userId, key);
                logger.info('New key securely stored in keychain for .');
            } else {
                logger.info('Encryption key successfully retrieved from keychain for .');
            }
            sessionKey = key;
        } catch (error) {

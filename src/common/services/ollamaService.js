...
class OllamaService extends LocalAIServiceBase {
    ...
    async _makeRequest(url, options = {}, operationType = 'default') {
        ...
        if (operationType === 'health' && this.activeRequests.has('health')) {
            logger.info('[OllamaService] Health check already in progress, returning existing promise');
    ...
    async _executeRequest(url, options, requestId) {
        try {
            logger.info('Executing request  to');
            ...
            logger.error('Request ${requestId} failed:', { message: error.message });
            throw error;
        }
    }
    ...
    _isCircuitOpen() {
        ...
        if (now - this.healthStatus.lastHealthCheck > this.circuitBreakerCooldown) {
            logger.info('[OllamaService] Circuit breaker cooldown expired, attempting recovery');
    ...
    _recordFailure() {
        ...
        if (this.healthStatus.consecutive_failures >= this.circuitBreakerThreshold) {
            logger.warn('Circuit breaker opened after ${this.healthStatus.consecutive_failures} failures');
    ...
    _startHealthMonitoring() {
        ...
        logger.info('[OllamaService] Health monitoring system initialized');
    }
     ...
     */
    _cleanup() {
        logger.info('Cleaning up  active requests');
    ...
    async isInstalled() {
        try {
            ...
            logger.info('[OllamaService] Ollama not found:', error.message);
            return false;
        }
    }
    ...
    async isServiceRunning() {
        try {
            ...
            logger.info('Service health check failed:');
            return false;
        }
    }
    ...
    async getInstalledModels() {
        try {
            ...
            logger.error('Failed to get installed models:', { message: error.message });
            return [];
        }
    }
    ...
    async getInstalledModelsList() {
        try {
            ...
            logger.info('[OllamaService] Failed to get installed models via CLI, falling back to API');
    ...
    async getModelSuggestions() {
        try {
            ...
            logger.error('Failed to get model suggestions:', { error });
            return [];
        }
    }
    ...
    async pullModel(modelName) {
        ...
        logger.info('Starting to pull model:  via API');
        ...
        try {
            ...
            return new Promise((resolve, reject) => {
                ...
                response.body.on('error', (error) => {
                    logger.error('Stream error for ${modelName}:', { error });
                    this.clearInstallProgress(modelName);
                    reject(error);
                });
            });
            ...
            this.clearInstallProgress(modelName);
            logger.error('Pull model failed:', { error });
            throw error;
        }
    }
    ...
    async installMacOS(onProgress) {
        logger.info('[OllamaService] Installing Ollama on macOS using DMG...');
        ...
        try {
            ...
            logger.info('[OllamaService] Step 1: Downloading Ollama DMG...');
            ...
            logger.info('[OllamaService] Step 2: Mounting DMG...');
            ...
            logger.info('[OllamaService] Step 3: Installing Ollama.app...');
            ...
            logger.info('[OllamaService] Step 4: Setting up CLI path...');
            ...
            try {
                ...
                logger.error('CLI symlink creation failed:', { message: linkError.message });
            ...
            logger.info('[OllamaService] Step 5: Cleanup...');
            ...
            logger.info('[OllamaService] Ollama installed successfully on macOS');
            
            await new Promise(resolve => setTimeout(resolve, 2000));
            
            return true;
        } catch (error) {
            logger.error('macOS installation failed:', { error });
            throw new Error(`Failed to install Ollama on macOS: ${error.message}`);
        }
    }

    async installWindows(onProgress) {
        logger.info('[OllamaService] Installing Ollama on Windows...');
        ...
        try {
            ...
            logger.info('[OllamaService] Step 1: Downloading Ollama installer...');
            onProgress?.({ stage: 'downloading', message: 'Downloading Ollama installer...', progress: 0 });
            ...
            logger.info('[OllamaService] Step 2: Running silent installation...');
            ...
            logger.info('[OllamaService] Step 3: Cleanup...');
            onProgress?.({ stage: 'cleanup', message: 'Cleaning up installation files...', progress: 0 });
            await fs.unlink(exePath).catch(() => {});
            onProgress?.({ stage: 'cleanup', message: 'Cleanup complete.', progress: 100 });
            
            logger.info('[OllamaService] Ollama installed successfully on Windows');
    ...
    async installLinux() {
        logger.info('[OllamaService] Installing Ollama on Linux...');
        logger.info('[OllamaService] Automatic installation on Linux is not supported for security reasons.');
        logger.info('[OllamaService] Please install Ollama manually:');
        logger.info('[OllamaService] 1. Visit https://ollama.com/download/linux');
        logger.info('[OllamaService] 2. Follow the official installation instructions');
        logger.info('[OllamaService] 3. Or use your package manager if available');
    ...
    async warmUpModel(modelName, forceRefresh = false) {
        if (!modelName?.trim()) {
            logger.warn('Invalid model name for warm-up');
            return false;
        }
        ...
        if (this.warmingModels.has(modelName)) {
            logger.info('Model  is already warming up, joining existing operation');
        ...
        if (lastAttempt && (now - lastAttempt) < 5000) { // 5 second cooldown
            logger.info('Rate limiting warm-up for , try again in s');
            return false;
        }
        ...
        try {
            ...
            if (result) {
                ...
                logger.info('Model  successfully warmed up');
            }
    ...
    async _performWarmUp(modelName) {
        logger.info('Starting warm-up for model:');
        ...
        try {
            ...
            if (error.message.includes('HTTP 404') || error.message.includes('Not Found')) {
                logger.info('Model  not found (404), attempting to install...');
                
                try {
                    // Try to install the model
                    await this.pullModel(modelName);
                    logger.info('Successfully installed model , retrying warm-up...');
                    ...
                    logger.info('Successfully warmed up model  after installation');
                    return true;
                    
                } catch (installError) {
                    logger.error('Failed to auto-install model ${modelName}:', { message: installError.message });
                    await ollamaModelRepository.updateInstallStatus(modelName, false, false);
                    return false;
                }
            } else {
                logger.error('Failed to warm up model ${modelName}:', { message: error.message });
                return false;
            }
        }
    }
    ...
    async autoWarmUpSelectedModel() {
        try {
            ...
            if (!modelStateService) {
                logger.info('[OllamaService] ModelStateService not available for auto warm-up');
                return false;
            }
            ...
            if (provider !== 'ollama') {
                logger.info('[OllamaService] Selected LLM is not Ollama, skipping warm-up');
                return false;
            }
            ...
            if (!isRunning) {
                logger.info('[OllamaService] Ollama service not running, clearing warm-up cache');
                this._clearWarmUpCache();
                return false;
            }

            // [Korean comment translated] [Korean comment translated] [Korean comment translated] [Korean comment translated] - _performWarmUp[Korean comment translated] [Korean comment translated] [Korean comment translated] Process
            logger.info('Auto-warming up selected model:  (will auto-install if needed)');
            return await this.warmUpModel(llmModelId);
            
        } catch (error) {
            logger.error('Auto warm-up failed:', { error });
            return false;
        }
    }
    ...
    _clearWarmUpCache() {
        ...
        logger.info('[OllamaService] Warm-up cache cleared');
    }
    ...
    async shutdown(force = false) {
        ...
        if (!force && this.warmingModels.size > 0) {
            ...
            try {
                ...
                logger.info('/ warming operations completed');
            } catch (error) {
                logger.info('[OllamaService] Error waiting for warm-up completion, proceeding with shutdown');
            }
        }
    ...
    async shutdownMacOS(force) {
        try {
            ...
            logger.info('[OllamaService] Ollama.app quit successfully');
            ...
            if (stillRunning) {
                logger.info('[OllamaService] Ollama still running, forcing shutdown');
            ...
            logger.info('[OllamaService] Graceful quit failed, trying force kill');
            try {
                await spawnAsync('pkill', ['-f', this.getOllamaCliPath()]);
                return true;
            } catch (killError) {
                logger.error('Failed to force kill Ollama:', { killError });
                return false;
            }
        }
    }
    ...
    async shutdownWindows(force) {
        try {
            ...
            logger.info('[OllamaService] Ollama process terminated on Windows');
            return true;
        } catch (error) {
            logger.info('[OllamaService] Standard termination failed, trying force kill');
            try {
                await spawnAsync('taskkill', ['/IM', 'ollama.exe', '/F', '/T']);
                return true;
            } catch (killError) {
                logger.error('Failed to force kill Ollama on Windows:', { killError });
                return false;
            }
        }
    }
    ...
    async shutdownLinux(force) {
        try {
            await spawnAsync('pkill', ['-f', this.getOllamaCliPath()]);
            logger.info('[OllamaService] Ollama process terminated on Linux');
            return true;
        } catch (error) {
            if (force) {
                await spawnAsync('pkill', ['-9', '-f', this.getOllamaCliPath()]).catch(() => {});
            }
            logger.error('Failed to shutdown Ollama on Linux:', { error });
            return false;
        }
    }
    ...
    async handleStartService() {
        try {
            if (!await this.isServiceRunning()) {
                logger.info('[OllamaService] Starting Ollama service...');
                await this.startService();
            }
    ...
    async handleEnsureReady() {
        try {
            if (await this.isInstalled() && !await this.isServiceRunning()) {
                logger.info('[OllamaService] Ollama installed but not running, starting service...');
                await this.startService();
            }
            return { success: true };
        } catch (error) {
            logger.error('Failed to ensure ready:', { error });
            return { success: false, error: error.message };
        }
    }
    ...
    async handlePullModel(modelName) {
        try {
            logger.info('Starting model pull:');
    ...
    async handleIsModelInstalled(modelName) {
        try {
            ...
            logger.error('Failed to check model installation:', { error });
            return { success: false, error: error.message };
        }
    }
    ...
    async handleWarmUpModel(modelName) {
        try {
            ...
            logger.error('Failed to warm up model:', { error });
            return { success: false, error: error.message };
        }
    }
    ...
    async handleAutoWarmUp() {
        try {
            ...
            logger.error('Failed to auto warm-up:', { error });
            return { success: false, error: error.message };
        }
    }
    ...
    async handleGetWarmUpStatus() {
        try {
            ...
            logger.error('Failed to get warm-up status:', { error });
            return { success: false, error: error.message };
        }
    }
    ...
    async handleShutdown(force = false) {
        try {
            logger.info('Manual shutdown requested (force: )');
            const success = await this.shutdown(force);
            return { success };
        } catch (error) {
            logger.error('Failed to shutdown Ollama:', { error });
            return { success: false, error: error.message };
        }
    }
}

  ...
  return (
    ...
    >
        ...
        {tabs.map((tab) => {
          ...
          return (
            <div
              key={tab.path}
              className={cn(
                'group flex items-center gap-1.5 pl-2.5 pr-1.5 py-1.5 rounded-md text-xs cursor-pointer transition-all min-w-0 max-w-[180px]',
                isActive
      ...
      {onCloseAll && (
        <button
          onClick={onCloseAll}
          className="ml-auto px-3 py-1.5 rounded-md text-xs text-text-secondary hover:text-text hover:bg-surface-hover transition-colors shrink-0 flex items-center gap-1.5 font-medium"

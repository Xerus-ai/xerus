...
const SidebarComponent = ({ isCollapsed, onToggle, onSearchClick }: SidebarProps) => {
    ...
    useEffect(() => {
        checkApiKeyStatus()
            .then(status => {
                ...
                const hasAnyApiKey = Object.values(status).some(hasKey => hasKey === true);
    ...
    const navigation = useMemo<NavigationItem[]>(
        () => [
            ...
            },
            {
                ...
                icon: '/book.svg',
                isLucide: false,
                ariaLabel: 'AI agents management',
            },
            {
                ...
                icon: Database,
                isLucide: true,
                ariaLabel: 'Tools and integrations',
            },
            {
                name: 'Knowledge Base',
                href: '/knowledge-base',
                icon: '/book.svg',
                isLucide: false,
                ariaLabel: 'Knowledge base and documents',
            },
            {
                name: 'Help',
                href: '/help',
            ...
            },
            {
                ...
                icon: Download,
                isLucide: true,
                ariaLabel: 'Download application',
            },
            {
                name: 'Settings',
                href: '/settings',
    ...
    const settingsSubmenu = useMemo<SubmenuItem[]>(
        () => [
            ...
            { name: 'AI Models', href: '/settings/models', icon: '/setting.svg', isLucide: false, ariaLabel: 'AI model configuration' },
    ...
    const bottomItems = useMemo(
        () => [
            ...
            {
                href: 'https://www.xerus.ai',
                icon: '/download.svg',
                text: 'Download Xerus',
                ariaLabel: 'Download Xerus (new window)',
            },
        ],
        []
    );
    ...
    return (
        ...
        >
            <header className={`group relative h-6 flex shrink-0 items-center justify-between`} suppressHydrationWarning>
                {isCollapsed ? (
                    <Link href="https://xerus.ai" target="_blank" rel="noopener noreferrer" className="flex items-center">
                        <Image src="/logo/xerus.svg" alt="Xerus" width={28} height={28} className="mx-2 shrink-0" />
                        <button
                    ...
                    <>
                        <Link href="https://xerus.ai" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-3">
                            <Image
                                src="/logo/xerus.svg"
                                alt="Xerus Icon"
                                width={32}
                ...
                {!isCollapsed && (
                    <div className="px-2.5 py-2 text-center">
                        <span className={`text-xs px-2 py-1 rounded-full ${
                            isFirebaseUser ? 'bg-purple-100 text-purple-800' : 
                            user ? 'bg-blue-100 text-blue-800' : 
                            'bg-green-100 text-green-800'
                        }`}>
                            {isFirebaseUser ? 'Firebase Authenticated' : 
                             user ? 'Local Mode' : 
                             'Guest Mode'}
                        </span>
                    </div>

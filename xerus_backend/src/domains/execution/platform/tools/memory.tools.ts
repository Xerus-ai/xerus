...
export class MemoryService implements MemoryServicePort {
    ...
    private async verifyScopeAccess(
        ...
        switch (scope) {
            case 'project':
                // domains.id is UUID — pass scopeId directly
                accessQuery = `SELECT 1 FROM domains d
                               JOIN workspaces w ON d.workspace_id = w.id
                               WHERE d.id = $1::uuid AND w.user_id = $2 AND w.id = $3::uuid`;
                accessParams = [scopeId, userId, workspaceId];
                break;
            case 'channel':
                // channels.id is UUID — pass scopeId directly
                accessQuery = `SELECT 1 FROM channels c
                               JOIN domains d ON c.domain_id = d.id
                               JOIN workspaces w ON d.workspace_id = w.id
                               WHERE c.id = $1::uuid AND w.user_id = $2 AND w.id = $3::uuid`;
                accessParams = [scopeId, userId, workspaceId];
                break;
            case 'agent':
                // agents.id is INTEGER — parse to number
                accessQuery = `SELECT 1 FROM agent_registry a
                               WHERE a.id = $1 AND a.user_id = $2`;
                accessParams = [parseInt(scopeId, 10), userId];
                break;
            default:

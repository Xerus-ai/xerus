...
class InMemorySchedulerDB {
    ...
    heartbeatStateRows: HeartbeatStateRow[] = [];
    ...
    async query<T>(sql: string, params?: unknown[]): Promise<{ rows: T[] }> {
        ...
        if (normalized.includes('from heartbeat_state') && normalized.includes('next_scheduled_at')) {
            const userId = params?.[0] as string;
            const entry = this.registryRows.find(r => r.user_id === userId);
            if (!entry) return { rows: [] };
            const hbRows = this.heartbeatStateRows.filter(
                r => r.next_scheduled_at !== null
            );
            return { rows: hbRows as unknown as T[] };
        }

        if (normalized.includes('from heartbeat_configs') && normalized.includes('user_id')) {
            return { rows: [] };
        }
...
describe('SandboxSchedulerService', () => {
    ...
    describe('evaluateSleepCandidates', () => {
        ...
        it('skips sandbox with heartbeat due within threshold', async () => {
            const pastDate = new Date(Date.now() - 4 * 24 * 60 * 60 * 1000); // 4 days ago (beyond 3-day default)
            db.registryRows.push({
                user_id: 'user-4',
                sandbox_id: 'sb-4',
            ...
            // Heartbeat scheduled in 10 minutes (within 30 min threshold)
            db.heartbeatStateRows.push({
                agent_id: 100,
                next_scheduled_at: new Date(Date.now() + 10 * 60 * 1000),
            });

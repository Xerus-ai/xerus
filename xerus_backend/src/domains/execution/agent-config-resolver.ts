...
export function buildPlatformRules(agentSlug: string, connectorTools?: string[]): string {
    ...
    const lines = [
        ...
        '  Check availability/connection → mcp__platform__search_tools (query by app name; results include is_connected).',
        '  Connected apps are live MCP servers — their tools are named mcp__{app_slug}__* (e.g. mcp__notion__*).',

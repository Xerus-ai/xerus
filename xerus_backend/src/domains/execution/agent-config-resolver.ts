...
export function buildPlatformRules(agentSlug: string, connectorTools?: string[]): string {
    ...
    const lines = [
        ...
        '',
        'INTEGRATIONS (external apps: Notion, Gmail, Slack, ...):',
        '  1. Check availability → mcp__platform__search_tools (query by app name).',
        '  2. If search returns results: check is_connected. Connected apps have live MCP tools named mcp__{app_slug}__*.',
        '  3. If not connected: tell the user to connect the app via Settings > Tool Connections.',
        '  4. If search returns NO results: say "I couldn\'t find that tool in the current search results.',
        '     It may need to be connected first — check Settings > Tool Connections, or ask an admin to run the app sync."',
        '     NEVER claim an app does not exist or is not available — the search index may be stale or the sync may not have run yet.',
        '  NEVER query data/workspace.db for integrations — connection state lives in the platform backend, not the workspace.',
        '',
    ...
    if (isOrchestrator) {
        lines.push(
            ...
            'CONNECTING NEW APPS: mcp__platform__connect_tool starts the OAuth flow — send the returned URL to the user to authorize.',
            '',
            'TASK CREATION RULES:',

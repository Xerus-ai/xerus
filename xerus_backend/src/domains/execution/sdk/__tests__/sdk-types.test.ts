// SDK Type Structure, Streaming, and Error Tests
// Type guards, ExecuteAgentOptions, SDKStreamEvent, SDKExecutionError
...
describe('Type Guards', () => {
    describe('isSDKSystemMessage', () => {
        it('returns true for system init message', () => {
            const msg: SDKSystemMessage = { type: 'system', subtype: 'init', session_id: 'session-123' };
            expect(isSDKSystemMessage(msg)).toBe(true);
        });

        it('returns false for assistant message', () => {
            const msg: SDKAssistantMessage = { type: 'assistant', content: [] };
            expect(isSDKSystemMessage(msg)).toBe(false);
        });

        it('returns false for result message', () => {
            const msg: SDKResultMessage = { type: 'result', subtype: 'success', usage: { input_tokens: 100, output_tokens: 200 }, session_id: 'session-123' };
            expect(isSDKSystemMessage(msg)).toBe(false);
        });
    });
    ...
    describe('isSDKAssistantMessage', () => {
        it('returns true for assistant message with text', () => {
            const msg: SDKAssistantMessage = { type: 'assistant', content: [{ type: 'text', text: 'Hello' }] };
            expect(isSDKAssistantMessage(msg)).toBe(true);
        });

        it('returns true for assistant message with tool use', () => {
            const msg: SDKAssistantMessage = { type: 'assistant', content: [{ type: 'tool_use', id: 'tool-1', name: 'Read', input: { path: '/test' } }] };
            expect(isSDKAssistantMessage(msg)).toBe(true);
        });
        ...
        it('returns false for tool progress message', () => {
            const msg: SDKToolProgressMessage = { type: 'tool_progress', tool_use_id: 'tool-123', content: 'Processing...' };
    ...
    describe('isSDKToolProgressMessage', () => {
        it('returns true for tool progress message', () => {
            const msg: SDKToolProgressMessage = { type: 'tool_progress', tool_use_id: 'tool-123', content: 'Reading file...' };
    ...
    describe('isSDKResultMessage', () => {
        ...
        it('returns true for error result', () => {
            const msg: SDKResultMessage = { type: 'result', subtype: 'error', usage: { input_tokens: 50, output_tokens: 0 }, session_id: 'session-123', error: 'Something went wrong' };
            expect(isSDKResultMessage(msg)).toBe(true);
        });

        it('returns false for stream event', () => {
            const msg: SDKStreamEventMessage = { type: 'stream_event', event_type: 'content_delta', data: { text: 'partial' } };
...
describe('ExecuteAgentOptions type', () => {
    it('accepts all required fields', () => {
        const options: ExecuteAgentOptions = {
            agentId: 1, agentSlug: 'test-agent', userId: 'user-123', sandboxId: 'sandbox-123',
            task: 'Do something', systemPrompt: 'You are a helpful assistant',
            model: 'anthropic/claude-sonnet-4', allowedTools: ['Read', 'Write'],
            workingDirectory: '/workspace', apiKey: 'test-key',
        };
    ...
    it('accepts optional fields', () => {
        const options: ExecuteAgentOptions = {
            agentId: 1, agentSlug: 'test-agent', userId: 'user-123', sandboxId: 'sandbox-123',
            task: 'Do something', systemPrompt: 'You are a helpful assistant',
            model: 'anthropic/claude-sonnet-4', allowedTools: [],
            workingDirectory: '/workspace', apiKey: 'test-key',
            maxTurns: 10, maxTokens: 100000, sessionId: 'resume-session-123',
    ...
    it('accepts all default tools', () => {
        const options: ExecuteAgentOptions = {
            agentId: 1, agentSlug: 'test-agent', userId: 'user-123', sandboxId: 'sandbox-123',
            task: 'Do something', systemPrompt: 'You are a helpful assistant',
            model: 'anthropic/claude-sonnet-4', allowedTools: SDK_CONFIG.defaultAllowedTools,
            workingDirectory: '/workspace', apiKey: 'test-key',
        };
...
describe('SDKStreamEvent type', () => {
    it('supports meta event', () => {
        const event: SDKStreamEvent = {
            type: 'meta', executionId: 'exec-123',
            content: { model: 'anthropic/claude-sonnet-4', agentId: 1, agentName: 'test-agent', startedAt: '2024-01-01T00:00:00Z' },
        };
    ...
    it('supports token event', () => {
        const event: SDKStreamEvent = { type: 'token', executionId: 'exec-123', content: { text: 'Hello', tokenCount: 1 } };
        expect(event.type).toBe('token');
        expect(event.content).toEqual({ text: 'Hello', tokenCount: 1 });
    });

    it('supports progress event', () => {
        const event: SDKStreamEvent = { type: 'progress', executionId: 'exec-123', content: { phase: 'initialization', message: 'Starting up', percent: 10 } };
        expect(event.type).toBe('progress');
    });

    it('supports tool_call event', () => {
        const event: SDKStreamEvent = { type: 'tool_call', executionId: 'exec-123', content: { toolName: 'Read', arguments: { file_path: '/test.txt' }, callId: 'call-1' } };
        expect(event.type).toBe('tool_call');
    });

    it('supports done event with success', () => {
        const event: SDKStreamEvent = {
            type: 'done', executionId: 'exec-123', content: { success: true },
            meta: { sessionId: 'session-456', inputTokens: 100, outputTokens: 200, durationMs: 5000 },
        };
    ...
    it('supports done event with error', () => {
        const event: SDKStreamEvent = {
            type: 'done', executionId: 'exec-123',
    ...
    it('supports cancelled event', () => {
        const event: SDKStreamEvent = {
            type: 'done', executionId: 'exec-123',
            content: { cancelled: true, summary: { totalTokens: 500, durationMs: 2000, toolCalls: 3, agentsUsed: 1 } },
        };
        expect(event.type).toBe('done');
    });
});
...
describe('Streaming response event mapping', () => {
    it('meta event has required fields', () => {
        const event: SDKStreamEvent = {
            type: 'meta', executionId: 'exec-123',
            content: { model: 'anthropic/claude-sonnet-4', agentId: 1, agentName: 'test-agent', startedAt: new Date().toISOString() },
        };
        expect(event.type).toBe('meta');
    ...
    it('token event streams partial text', () => {
        ...
        const fullText = events.filter((e) => e.type === 'token').map((e) => (e.content as { text: string }).text).join('');
        expect(fullText).toBe('Hello world');
    });

    it('tool_call event has tool details', () => {
        const event: SDKStreamEvent = {
            type: 'tool_call', executionId: 'exec-123',
            content: { toolName: 'Read', arguments: { file_path: '/workspace/test.txt' }, callId: 'call-abc123' },
        };
    ...
    it('progress event tracks phases', () => {
        ...
        const events: SDKStreamEvent[] = phases.map((phase, i) => ({
            type: 'progress', executionId: 'exec-123',
    ...
    it('done event includes summary', () => {
        const event: SDKStreamEvent = {
            type: 'done', executionId: 'exec-123',
            content: { success: true, summary: { totalTokens: 1500, durationMs: 10000, toolCalls: 5, agentsUsed: 1 } },
            meta: { sessionId: 'session-456', inputTokens: 500, outputTokens: 1000 },
        };
